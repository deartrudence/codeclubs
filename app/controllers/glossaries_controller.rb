  ...
  def index
    ...
    if params[:single_letter]
      if params[:single_letter][:letter] == '#'
        ...
        @glossaries = Glossary.where("LOWER(term) like ?", "#{params[:single_letter][:letter].downcase}%").paginate(:page => params[:page], :per_page => 10).by_language(lang).order('LOWER(term) asc')
      end
      respond_to do |format|
        format.js { render :partial => "glossary_list_js" }
      end
