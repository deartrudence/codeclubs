  ...
  def index
    ...
    if params[:alphabetical]
      if params[:alphabetical][:terminology] == '1'
        @glossaries = Glossary.paginate(:page => params[:page], :per_page => 50).by_language(lang).order('LOWER(term) desc')
      elsif params[:alphabetical][:terminology] == '0'
        @glossaries = Glossary.paginate(:page => params[:page], :per_page => 50).by_language(lang).order('LOWER(term) asc')
      end
      respond_to do |format|
        format.js { render :partial => "glossary_list_js" }
      end
    else
       @glossaries = Glossary.paginate(:page => params[:page], :per_page => 50).by_language(lang).order('term asc')
    end
    ...
    if params[:single_letter]
      if params[:single_letter][:letter] == '#'
        ...
        @glossaries = Glossary.where("LOWER(term) like ?", "#{params[:single_letter][:letter].downcase}%").paginate(:page => params[:page], :per_page => 50).by_language(lang).order('LOWER(term) asc')
      end
      respond_to do |format|
        format.js { render :partial => "glossary_list_js" }
      end
